...
public class PasswordEvaluator {
	...
	public String evaluatePassword(String input) {
		...
		String errMessage = "";
		if (!foundUpperCase)
			errMessage += "Upper case char; ";
		
		if (!foundLowerCase)
			errMessage += "Lower case char; ";
		
		if (!foundNumericDigit)
			errMessage += "Numeric digit; ";
			
		if (!foundSpecialChar)
			errMessage += "Special char; ";
			
		if (!foundLongEnough)
			errMessage += "Minimum 8 characters";
		
		if (errMessage == "")
			return "";
		
		passwordIndexofError = currentCharNdx;
		return "The following conditions were not satisfied:\n" +  errMessage;
